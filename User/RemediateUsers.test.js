/*!
     * Copyright 2017-2017 Mutual of Enumclaw. All Rights Reserved.
     * License: Public
*/ 

//Test function for RemediateUsers.js

const config = require('./config.json');

//Environment variable values defined from the config.json file.
Object.keys(config).forEach((key) => {
    process.env[key] = config[key];
})

const main = require(`./RemediateUsers`); 
const Master = require('./MasterClass').handler;
const getEvent = require('./EventCreater').handler;
let master = new Master();
let path = require("./MasterClass").path;

//overriding the ses.sendEmail operation to just log the html snippet
master.setSes((params) => {
    let html = params.Message.Body.Html.Data;
    html = html.replace('data:image/jpeg;base64,', 'annoying image link');
    console.log(html);
    return {promise: () => {} }
})

master.setDynamo((params) => {
    return {promise: () => {} };
});


//*******remediationTest() parameter examples**********
//event > the event that is passed to the main function. (Call getEvent())
//functionName > "detachRolePolicy" (no cap on first letter). The aws iam function you 
//want to override

//_____________getEvent() parameters for reference_____________
//env, launch, eventName, requestParameters, responseElements

describe(`Remediate Users`, () => {

    // test calls ************* CHANGE *************
    test(`Testing PutUserPolicy event`, async() => {
        await remediationTest(getEvent('dev', "console", 'PutUserPolicy'), "deleteUserPolicy");
    });
    // test(`Testing AttachUserPolicy event`, async() => {
    //     await remediationTest(getEvent('snd', "console", 'AttachUserPolicy'), 'detachUserPolicy');
    // });
    // test(`Testing DetachUserPolicy event`, async() => {
    //     await remediationTest(getEvent('dev', "console", 'DetachUserPolicy'), 'attachUserPolicy');
    // });
    // test('Testing DeleteUserPolicy Event.', async() => {
    //     await remediationTest(getEvent('dev', "console", 'DeleteUserPolicy'), 'deleteUserPolicy');
    // });
    // test(`Testing CreateUser event`, async() => {
    //     await remediationTest(getEvent('snd', "console", 'CreateUser'), 'deleteUser');
    // });
    // test(`Testing DynamoEvent event`, async() => {
    //     await remediationTest(getEvent('dev', "console", 'Dynamo'), 'deleteUser');
    // });
    // test('Testing DeleteUser Event.', async() => {
    //     await remediationTest(getEvent('snd', "console", 'DeleteUser'), 'deleteUser')
    // });
    // test(`Testing AddUserToGroup event`, async() => {
    //     await remediationTest(getEvent('snd', "console", 'AddUserToGroup'), 'removeUserFromGroup');
    // });
    // test(`Testing RemoveUserFromGroup event`, async() => {
    //     await remediationTest(getEvent('dev', "console", 'RemoveUserFromGroup'), 'addUserToGroup');
    // });
});

//test function
async function remediationTest(event, functionName) {

    //overriding the iam function being called so it just checks for the correct params
    await main.setFunction(functionName, (params) => {

            return {promise: () => {} }
    });
    
    await main.setFunction('deleteUserPolicy', (params) => {

        return {promise: () => {} }
    });

    await main.setFunction('detachUserPolicy', (params) => {

        return {promise: () => {} }
    });

    await main.setFunction('removeUserFromGroup', (params) => {

        return {promise: () => {} }
    });

    await main.setSqs('sendMessage', (params) => {
        return {promise: () => {} };
    });

    await main.handler(event); 
}